...
void MatrizDispersa::generarReporteGraphviz(std::string tituloFuncion) {
    ...
    for (int f = 1; f <= totalFilas; ++f) {
        for (int c = 1; c <= totalColumnas; ++c) {
            ...
            if (asiento != nullptr) {
                archivo << " [label=\"Reservado"
                        << "\\nF" << f << "-C" << c
                        << "\", shape=box, style=\"rounded,filled\", width=0.72, height=0.42, fixedsize=true, fillcolor=\"#ff6b6b\", color=\"#37474f\", penwidth=1.4, fontsize=8];\n";
            } else {
                archivo << " [label=\"F" << f << "-C" << c
                        << "\\nLibre\", shape=box, style=\"rounded,dashed,filled\", width=0.72, height=0.42, fixedsize=true, fillcolor=\"#fff1f1\", color=\"#cfcfcf\", fontcolor=\"#777777\", fontsize=8];\n";
            }
        }
    }
